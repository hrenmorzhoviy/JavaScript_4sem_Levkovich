...
const findAll = (specialty, minScore, fullName) => {
    ...
    if (specialty) {
        applicants = applicants.filter(a => 
            a.specialty.toLowerCase().includes(specialty.toLowerCase())
        );
    }
    if (minScore) {
        const min = parseInt(minScore);
        applicants = applicants.filter(a => {
            const avg = (a.examScores.math + a.examScores.physics + a.examScores.russian) / 3;
            return avg >= min;
        });
    }
    if (fullName) {
        applicants = applicants.filter(a => 
            a.fullName.toLowerCase().includes(fullName.toLowerCase())
        );
    }
    return applicants;
};
...
const create = (applicantData) => {
    ...
    const newApplicant = {
        id: newId,
        ...applicantData,
        status: applicantData.status || "на рассмотрении"
    };
